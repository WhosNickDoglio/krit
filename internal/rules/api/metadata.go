...
//
// Rule authors call these instead of writing untyped Apply closures by hand.
// The constructors build the heterogeneous ConfigOption value (so the
// Options slice can hold mixed types) but the user-supplied Apply closure
// works on concrete pointer-to-rule and value types — no manual downcasts,
// no untyped value assertions, and a compile-time check that the rule field
// matches the option type.
...
//
// A nil Default is preserved as nil in the descriptor (the schema generator
// suppresses empty defaults). Use a non-nil zero-length slice if you need
// the schema to publish an explicit empty default.
...
//
// Default is the documentation default published in the schema; the runtime
// only invokes Apply when a config override is present and successfully
// compiles. Default is intentionally a raw pattern string to match the
// schema's "default" emission shape.
...
func RegexOption[R any](s RegexOptionSpec[R]) ConfigOption {
	return ConfigOption{
		...
		Description: s.Description,
		Apply: func(target interface{}, value interface{}) {
			s.Apply(target.(*R), value.(*regexp.Regexp))
		},
	}
}

// TypedCustomApply wraps a typed CustomApply closure for use in
// RuleDescriptor.CustomApply. It downcasts the target to *R before invoking
// the supplied function.
func TypedCustomApply[R any](apply func(*R, ConfigSource)) func(interface{}, ConfigSource) {
	return func(target interface{}, cfg ConfigSource) {
		apply(target.(*R), cfg)
	}
}
...
// ConfigSource is the abstraction the rule metadata runtime uses to read
...
// when those anchors are missing. Patterns are full-string matches rather than
// substring matches. Invalid patterns log a warning to stderr and return nil —
// callers must treat nil as "leave the existing field alone".
func CompileAnchoredPattern(ruleName, field, pattern string) *regexp.Regexp {
	...
	if err != nil {
		reporter().Warnf("krit: invalid regex in config %s.%s: %q: %v\n", ruleName, field, pattern, err)
		return nil
	}
	return compiled
}
