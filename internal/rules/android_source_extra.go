...
func viewTagReceiverEvidence(ctx *api.Context, target semantics.CallTarget) (float64, bool) {
	if target.Resolved {
...
func viewTagArgumentIsFrameworkHeavy(ctx *api.Context, expr uint32) bool {
...
func viewTagFrameworkTypeMatches(ctx *api.Context, typ *typeinfer.ResolvedType, targets []string) bool {
	...
	if ctx.Resolver == nil || ctx.File == nil || typ.Name == "" {
		return false
	}
...
func viewTagSameFileClassTypeMatches(ctx *api.Context, typ *typeinfer.ResolvedType, targets []string) bool {
	if ctx.Resolver == nil || ctx.File == nil || typ == nil || typ.Kind == typeinfer.TypeUnknown {
		return false
	}
...
func viewTagTypeMatches(ctx *api.Context, typ *typeinfer.ResolvedType, targets []string) bool {
	if typ == nil || typ.Kind == typeinfer.TypeUnknown {
		return false
	}
...
func viewTagClassHierarchyMatches(ctx *api.Context, typeName string, targets []string, visited map[string]bool) bool {
	if ctx.Resolver == nil || typeName == "" {
		return false
	}
...
func viewTagClassInfoMatches(ctx *api.Context, info *typeinfer.ClassInfo, targets []string, visited map[string]bool) bool {
	if info == nil {
		return false
	}
	key := info.FQN
...
// is still wrong.
func (r *WrongImportRule) check(ctx *api.Context) {
...
func missingPermissionRequiredAPI(ctx *api.Context, call uint32) (permissionAPI, semantics.SemanticEvidence, bool) {
...
func missingPermissionHasAnnotatedSameFileCandidate(ctx *api.Context, call uint32, callee string) bool {
	if ctx.File == nil || call == 0 || callee == "" {
		return false
	}
	file := ctx.File
	if !missingPermissionFileHasRequiresPermissionAnnotation(file) {
		return false
	}
	found := false
...
func missingPermissionReceiverDeclarationHasType(ctx *api.Context, receiver uint32, types []string) bool {
	if ctx.File == nil || receiver == 0 || len(types) == 0 {
		return false
	}
	file := ctx.File
...
func missingPermissionAnnotatedSameFileTarget(ctx *api.Context, call uint32, target semantics.CallTarget) (permissionAPI, bool) {
	if ctx.File == nil || target.CalleeName == "" {
...
func missingPermissionEnclosingPermissionCondition(ctx *api.Context, call uint32, perm string) bool {
	file := ctx.File
...
func missingPermissionConditionPermissionState(ctx *api.Context, cond uint32, perm string) missingPermissionGuardState {
	if ctx.File == nil || cond == 0 {
...
func missingPermissionEqualityPermissionState(ctx *api.Context, expr uint32, perm string) missingPermissionGuardState {
	file := ctx.File
...
func missingPermissionCheckSelfPermissionCall(ctx *api.Context, call uint32, perm string) bool {
	if ctx.File == nil || ctx.File.FlatType(call) != "call_expression" {
		return false
	}
	target, ok := semantics.ResolveCallTarget(ctx, call)
...
func missingPermissionHasPrecedingGrantedReturnGuard(ctx *api.Context, call uint32, perm string) bool {
...
func missingPermissionNodeTerminates(file *scanner.File, node uint32) bool {
	...
	case "call_expression":
		target, ok := semantics.ResolveCallTarget(&api.Context{File: file}, node)
...
func missingPermissionEnclosingAnnotationRequires(ctx *api.Context, call uint32, perm string) bool {
...
func missingPermissionFileHasRequiresPermissionAnnotation(file *scanner.File) bool {
	...
	found := false
	file.FlatWalkNodes(0, "annotation", func(annotation uint32) {
		if !found && missingPermissionAnnotationContainerHasRequiresPermission(file, annotation) {
			found = true
		}
	})
	return found
...
func missingPermissionAnnotationContainerHasRequiresPermission(file *scanner.File, container uint32) bool {
	if file == nil || container == 0 {
		return false
	}
	found := false
	file.FlatWalkAllNodes(container, func(candidate uint32) {
		...
		switch file.FlatType(candidate) {
		case "user_type", "type_identifier", "simple_identifier", "navigation_expression":
			path := missingPermissionIdentifierPath(file, candidate)
			if path == "" {
				path = file.FlatNodeText(candidate)
			}
			if missingPermissionSimpleName(path) == "RequiresPermission" {
				found = true
			}
		}
	})
...
func missingPermissionEnclosingCatchesSecurityException(ctx *api.Context, call uint32) bool {
...
func missingPermissionExprMatchesPermission(ctx *api.Context, expr uint32, perm string) bool {
...
func missingPermissionExprPermissionName(ctx *api.Context, expr uint32) (string, bool) {
	if ctx.File == nil || expr == 0 {
		return "", false
	}
...
func wrongConstantAllowedConstants(ctx *api.Context, call uint32, entry wrongConstantEntry) (wrongConstantAllowedSet, bool) {
...
func wrongConstantOracleCallTarget(ctx *api.Context, idx uint32) string {
	if ctx.Resolver == nil || ctx.File == nil {
		return ""
	}
...
func wrongConstantSameFileAllowedConstants(ctx *api.Context, call uint32, method string) (wrongConstantAllowedSet, bool) {
	file := ctx.File
...
func wrongConstantReceiverClassName(ctx *api.Context, receiver uint32) string {
	file := ctx.File
	if ctx.Resolver != nil {
	...
	decl := nullflow.ResolveSimpleReferenceDeclaration(file, receiver)
	if decl == 0 {
		return ""
	}
...
func wrongConstantEvalIntExpr(file *scanner.File, idx uint32, consts map[string]int64) (int64, bool) {
	...
	case "simple_identifier":
		...
		return value, ok
	case "navigation_expression":
		path, ok := nullflow.FlatReferencePathFromExpr(file, idx)
		if !ok {
			return 0, false
		}
