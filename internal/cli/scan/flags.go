...
func registerScanFlags(fs *flag.FlagSet) *scanFlags {
	...
	f.CPUProfile = fs.String("cpuprofile", "", "Write CPU profile to file")
	f.MemProfile = fs.String("memprofile", "", "Write memory profile to file")
	f.ProfileDispatch = fs.Bool("profile-dispatch", false, "Debug: emit per-file dispatch timing distribution to stderr")
